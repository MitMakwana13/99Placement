...
import { Sparkles, Trophy, CheckCircle, HelpCircle, XCircle, Bot, Loader2 } from "lucide-react";
...
export function ScreeningScorecardForm({
  ...
  const handleAiEvaluate = async () => {
    ...
    if (!currentNotes || currentNotes.trim().length < 20) {
      toast("Please provide at least a few sentences of internal feedback/notes before using AI evaluation.", "error");
      return;
    }

    setIsAiEvaluating(true);
    try {
      const res = await apiClient.post<any>("/ai/evaluate-notes", {
        notes: currentNotes,
        candidateRole: "Professional" 
      });
      
      setValue("communicationScore", res.communicationScore || 5, { shouldValidate: true });
      setValue("technicalScore", res.technicalScore || 5, { shouldValidate: true });
      setValue("experienceScore", res.experienceScore || 5, { shouldValidate: true });
      setValue("salaryAlignScore", res.salaryAlignScore || 5, { shouldValidate: true });
      setValue("noticePeriodScore", res.noticePeriodScore || 5, { shouldValidate: true });
      setValue("personalityScore", res.personalityScore || 5, { shouldValidate: true });
      setValue("verdict", res.verdict || "HOLD", { shouldValidate: true });
      setValue("recommendation", res.recommendation || "Generated by AI based on interviewer notes.", { shouldValidate: true });
      
      toast("AI evaluation completed successfully!", "success");
    } catch (err: any) {
      toast(err.message || "Failed to generate AI evaluation.", "error");
    } finally {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex items-center justify-between border-t border-border/20 pt-4 mt-2">
        <Button
          ...
          variant="outline"
          className="rounded-xl px-5 text-xs h-11 cursor-pointer flex items-center gap-1.5 border-primary/20 hover:bg-primary/5 text-primary"
        >
          {isAiEvaluating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}
          <span>{isAiEvaluating ? "AI is Evaluating..." : "Generate Scores via AI"}</span>
        </Button>
        <Button
          type="submit"
