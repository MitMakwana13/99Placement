...
export default function CandidateTestTakingPage() {
  ...
  if (isLoading) {
    return <div className="text-center py-20 text-xs font-semibold text-muted-foreground">Loading exam session...</div>;
  }

  if (!test) {
    return <div className="text-center py-20 text-xs text-muted-foreground">Exam session not found.</div>;
  }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full border rounded-3xl shadow-xl bg-card/50 backdrop-blur-md overflow-hidden">
          <div className="p-8 text-center space-y-6">
            ...
            <div className="p-4 bg-muted/40 border border-border/30 rounded-2xl grid grid-cols-2 gap-4">
              <div>
                <span className="text-[10px] font-black uppercase text-muted-foreground block">Correct answers</span>
                <span className="text-lg font-extrabold text-foreground">{test.correctAnswers} / {test.totalQuestions}</span>
              </div>
              <div>
                <span className="text-[10px] font-black uppercase text-muted-foreground block">Grade Score</span>
                <span className="text-lg font-extrabold text-foreground">{test.percentage}%</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <span className={`text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${
                ...
                Verdict: {isPassed ? "Pass / Shortlisted" : "Fail / Blocked"}
              </span>
              <p className="text-[11px] text-muted-foreground pt-2">
                {isPassed 
                  ? "Congratulations! You have satisfied the recruitment criteria. We will contact you shortly."
                  : "Thank you for participating. You did not meet the pass score benchmark for this recruitment role."
                }
              </p>
            </div>

            <Button
              onClick={() => router.push(`/dashboard/assessment/${test.id}`)}
              className="w-full rounded-xl text-xs h-11"
            >
              Return to Workspace
            </Button>
          </div>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between">
      ...
      {isOffline && (
        <div className="bg-red-500 text-white text-xs font-black py-2.5 px-6 flex items-center gap-2 justify-center animate-pulse border-b border-red-600">
          ...
          <span>INTERNET DISCONNECTED — DO NOT CLOSE OR REFRESH THIS WINDOW. PROGRESS WILL RE-SYNC UPON RECONNECTION.</span>
        </div>
      )}
      ...
      <footer className="border-t bg-card/45 backdrop-blur-md px-6 py-3 text-center text-[10px] text-muted-foreground font-semibold">
        © 99 Placement Recruiter Portal. Secure assessment engine.
      </footer>
